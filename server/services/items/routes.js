...
const {Item, Tag, User, Collection, ItemTags, Comment} = require('../../mysql/models')
...
route.post('/edit',isAuthorized, async(req, res) => {
    await Item.update({name: req.body.name, optionalFields: req.body.optionalFields}, {where: {id: req.body.id, CollectionId: req.body.CollectionId}})
    ...
    const ids = await ItemTags.findAll({where: {itemId: req.body.id}, attributes: ['id']})
    const flatIds = []

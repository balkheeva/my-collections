import {addField, deleteCollection, editCollection, getCollection, TCollection} from "../../api/collections";
...
export default function CollectionPage() {
    ...
    const handleInvalidate = () => {queryClient.invalidateQueries(queryKey)}
    const {data: collection} = useQuery<TCollection>(queryKey, () => getCollection(params.id))
    const collectionsMutationEdit = useMutation(editCollection, {onSuccess: handleInvalidate})
    const collectionsMutationDelete = useMutation(deleteCollection, {onSuccess: handleInvalidate})
    const {mutate, reset, error} = useMutation(createItem, {onSuccess: handleInvalidate})
    const itemMutationDelete = useMutation(deleteItem, {onSuccess: handleInvalidate})
        ...
        <div className="mb-4 bg-light rounded-3 shadow-sm p-4">
            <Row className="align-items-center mb-4">
                <Col xs={12} sm={6} md={5} lg={5} className="align-items-center mb-3">
                    <ImageContainer><Image src={collection.image ? collection.image : placeholderImage} rounded/></ImageContainer>
                </Col>
                <Col xs={12} sm={6} md={7} lg={7}>
                    <Stack direction="horizontal" gap={1}>
                        <h1>{collection.name}</h1>
                        {isAuthor && <SettingsDropDown
                            baseItem={collection}
                    ...
                    <Stack direction="horizontal" gap={3}>
                        {isAuthor && <Button variant="primary" onClick={() => setShowModalItem(true)}>Create new item</Button>}
                        {isAuthor && <Button onClick={() => setShowModalFields(true)}>Add fields</Button>}
                    </Stack>
                </Col>
            </Row>
           <SortFilterBar

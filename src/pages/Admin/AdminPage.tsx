                    ...
                    <td>
                        {user.id !== currentUser?.id && (
                            <Dropdown className="position-relative">
                                <Dropdown.Toggle variant="link" className="p-0"><SettingsIcon/></Dropdown.Toggle>
                                <Dropdown.Menu className="position-fixed" >
                                    <Dropdown.Item as="button" disabled={Boolean(currentUser?.impersonatedBy)} onClick={() => handleImpersonateUser(user)}>
                                        <ImpersonateIcon/> Impersonate
                                    </Dropdown.Item>
                                    <Dropdown.Item as="button" onClick={() => handleBlockOrUnblockUser(user)}>
                                        <BlockIcon/> {user.status === 'active' ? 'Block' : 'Unblock'}
                                    </Dropdown.Item>
                                    <Dropdown.Item as="button" onClick={() => handleChangeRole(user)}>
                                        <ChangeIcon/> Change role
                                    </Dropdown.Item>
                                    <Dropdown.Item as="button" onClick={() => handleDeleteUser(user.id)}>
                                        <DeleteIcon/> Delete
                                    </Dropdown.Item>
                                </Dropdown.Menu>
                            </Dropdown>

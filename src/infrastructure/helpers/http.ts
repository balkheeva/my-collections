export function post<
  ResponseBody extends Record<string, unknown> | Array<Record<string, unknown>>,
>(
  url: string,
...
function request<ResponseBody>(
  ...
  options: { withToken?: boolean; method: 'get' | 'post' },
): Promise<ResponseBody> {
  ...
  return (options.withToken === false ? callback() : withToken(callback)).then(
    handleResponse,
  );
}
...
function handleResponse(response: Response | typeof UNAUTHORIZED) {
  if (response === UNAUTHORIZED || response.status === 401) {
    ...
    return response.text().then((t: string) => Promise.reject(t));

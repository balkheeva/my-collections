...
export default function SearchDropDown({foundData, onClick}: any){
    return (
        <ListGroup
            ...
            onClick={onClick}
        >
            {foundData?.map((item: any) => <FoundItem key={item._id} item={item}/>)}
        </ListGroup>
    )
}
...
export function FoundItem(props: {item: any}) {
    ...
    return (
        <ListGroup.Item as={NavLink}  to={`/item/${item._id}`}>
            ...
            {fields.map((f, index) => <InnerHTML key={index} field={f} children={item} prettyFields={prettyFields}/>)}
        </ListGroup.Item>
    )
}

export function InnerHTML(props: {children: any, field: string, prettyFields: any}) {
    const {children, field, prettyFields} = props
    return <small>{prettyFields[field]}: <span dangerouslySetInnerHTML={{ __html: children?.highlight?.[field]?.[0] }}/> </small>
}
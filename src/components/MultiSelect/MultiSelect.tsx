import {Form, ListGroup, ListGroupItem, Button, InputGroup} from "react-bootstrap";
...
type Props = {
    ...
    onLoadItems: (search: string) => Promise<Array<{ value: any, label: string }>>
...
export default function MultiSelect(props: Props) {
    ...
    const filtered = items.filter(item => values?.every(i => i.value !== item.value))
    return (
        <>
            ...
            <div className="mt-1 mb-1">
                {values?.map((item) =>
                    <SelectedItem key={item.value} baseItem={item} onDeleteItem={handleDeleteItem}/>)}
            </div>
            {isOpen &&
...
export function SelectedItem(props: { baseItem: Item, onDeleteItem: (id: number) => void }) {
    const {baseItem, onDeleteItem} = props
    return (
        <Button size="sm" onClick={() => onDeleteItem(baseItem.value)} className="rounded-pill">
            {baseItem.label}
            <CrossIcon/>
        </Button>)
}